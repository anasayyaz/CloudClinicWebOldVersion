...
import { AddAPhoto, PermMedia, History } from "@material-ui/icons";
...
class Patients extends React.Component {
  ...
  componentDidMount() {
    // alert("on");
    list(`patient/patientsVisits/${this.props.match.params.id}`).then(
      (response) => {
        let data = [];
        if (typeof response.data == typeof "abc") {
          ...
          response.data.map((d) => {
            ...
            d.startDateTime = moment(d.startDateTime).format(
              "MMM DD, YYYY hh:mm A"
            );
            d.endDateTime = moment(d.endDateTime).format(
              "MMM DD, YYYY hh:mm A"
            );
            //  alert(d.startDateTime);
            //  alert(d.endDateTime);
            data.push(d);
          });
  ...
  render() {
    ...
    return (
      <div className="tableWrapper">
        ...
        />
        <STable
          title="Patients Previous Visits"
