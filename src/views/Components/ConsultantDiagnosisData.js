...
const ConsultantDiagnosisData = (props) => {
  ...
  const getAllICD = async () => {
    ...
    const getAll = await fetch(
      "https://cloudclinicdevapi.azurewebsites.net/api/diagnostic",
      {
        method: "GET",
        headers: {
          Authorization: `Bearer ${authToken}`,
  ...
  const getAllDiagnostic = async () => {
    const getAll = await fetch(
      `https://cloudclinicdevapi.azurewebsites.net/api/patientdiagnostic/diagnosticsByPatient/${props.patientID}`,
      {
        method: "GET",
        headers: {
          Authorization: `Bearer ${authToken}`,
  ...
  const submitAllDiagnosticHandler = async () => {
    for (let i = 0; i < localDiagnostic.length; i++) {
      await createDiagnostic(localDiagnostic[i]?.icd?.split(": ")[0]);
      console.log(localDiagnostic[i]?.icd?.split(": ")[0]);
    }

    getAllDiagnostic();
  };

  const createDiagnostic = async (icd) => {
    try {
      const body = {
        id: "0",
      ...
      const response = await fetch(
        "https://cloudclinicdevapi.azurewebsites.net/api/patientdiagnostic",
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${authToken}`,
    ...
    <div className="col-md-12 p-0  ">
      ...
      <div className="form-group">
        ...
        <ul class="m-0 px-1 text-left">
          ...
          >
            <p className="m-0">{translate("CURRENT_DIAGNOSIS")}</p>
          ...
          {localDiagnostic &&
            localDiagnostic.map((data, index) => (
              ...
              >
                ...
                <div className="pr-2 col-md-4">{data.icd}</div>
                ...
                <li className="d-flex align-items-center py-2 border-bottom w-100">
                  <div className="pr-1 pl-0 font-weight-bold col-md-10">
                    <input
                      ...
                      placeholder={translate("SELECT_DIAGNOSIS")}
                      ref={icdRef}
                      onChange={localDiagnosticHandler}
                    />
                    <datalist id="icd" className="cutom-select px-2 py-5">
                      {allICD &&
...
export default ConsultantDiagnosisData;
