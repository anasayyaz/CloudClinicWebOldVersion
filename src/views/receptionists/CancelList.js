...
class CancelList extends React.Component {
  ...
  handlePCareSearchClick(request, $class) {
    let requestData = request ? request : this.state.requestData;
    list("visit/cancelVisitsList", requestData).then((response) => {
      $class.setState({ rowDataPCP: response.data });
    })  .catch((error) => {
      if (error.response.data == "Record not found in system") {
        $class.setState({ rowDataPCP: error.response.data });
        toast.error("Record not found in system");
      }
    });
  }
...
export default connect(mapStateToProps, null)(CancelList);
