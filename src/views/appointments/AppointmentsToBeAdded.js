...
class AppointmentsToBeAdded extends React.Component {
  ...
  needAppointment(val) {
    ...
    list(`visit/${val}`).then((response) => {
      ...
      cid = response.data.consultant_NationalID;
      pname =
        response.data.patient.aspNetUser.firstName +
        " " +
        response.data.patient.aspNetUser.lastName;
      cname =
        response.data.physician.aspNetUser.firstName +
        " " +
        response.data.physician.aspNetUser.lastName +
        " ( " +
        response.data.physician.speciality +
        " )";

      if (
        response.data.isfollowup == false &&
        response.data.isConsultantRequired == false
      ) {
        type = "reschedule";
      } else if (
        response.data.isfollowup == true &&
        response.data.isConsultantRequired == false
      ) {
        type = "followup";
      } else if (
        response.data.isfollowup == false &&
        response.data.isConsultantRequired == true
      ) {
        ctype = response.data.referredTo;
        type = "consultation";
      ...
      setTimeout(() => {
        specialityData = this.state.physicianData.filter(function (el) {
          return el.speciality == ctype;
        });
  ...
  handlePCareSearchClick(request, $class) {
    let requestData = request ? request : this.state.requestData;
    list("/patient/patientsAppointmentList", requestData).then((response) => {
  ...
  getLatest(confirmStatus) {
    ...
    if (confirmStatus == "All") {
      list("patient/patientsAppointmentList/?pageNumber=1&pageSize=10&QuerySearch=&Action=")
      .then((response) => {
    ...
    return (
      <div className="tableWrapper">
        ...
        <div>
          ...
          <div className="tableWrapper">
            <PaginationTable
              title={translate("APPOINTMENTS_TO_BE_ADDED")}
...
export default connect(mapStateToProps, null)(AppointmentsToBeAdded);
