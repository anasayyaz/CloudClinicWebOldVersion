...
import { margin } from "@arction/lcjs";
...
export default class Physicians extends React.Component {
      ...
      columns: [
        {
          ...
          options: {
            customBodyRender: (val) => (
              <NavLink className="NavLink" to={`physicians/EditPhysician?Physician=${val}`}>
                {val}
              </NavLink>
            ),
          },
        ...
        },
        {
          ...
          hide:true,
          name: "nationalID",
          button:{show:true, value:'Edit', operation: (val) => (this.props.history.push(`/EditPhysician/${val}`)), icon:Edit},
        },
      ],
      rowSelection: "single",
      rowData: [],
  ...
  render() {
    ...
    let {requestData,search,pagination, rowData:{paginationMetadata} ,bulkUploadModal} = this.state;

    paginationMetadata?.currentPage && this.disableButtons();
    return (
      <PrintProvider ref={(el) => (this.componentRef = el)} className="tableWrapper">
        <NoPrint>
          <div  style={{ marginTop: "0px", marginBottom: "20px", textAlign: "right" }}>
        <NavLink
              className="col-md-1 px-1 w-100 border-0 shadow btn cc-btn"
            to={`/AddPhysician`} > {translate("ADD_NEW_PHYSICIAN")}
              </NavLink>
        ...
        <PopUp
                    $class={this}
                    buttons={[
                      { title: this.context.translate("CLOSE"), className: "btn btn-secondary", action: "setState" },
                      { title: this.context.translate("SAVE"), className: "btn cc-btn", action: "handleBulkUpload" },
                    ]}
                    ...
                    content={
                      ...
                      />
                       ...
                       <a className=" btn cc-btn float-left mt-1 float-right" href="avatars/PhysicianSampleFile.xlsx" download>{translate("DOWNLOAD_SAMPLE")}</a>
