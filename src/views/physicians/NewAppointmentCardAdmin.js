...
const NewAppointmentCardAdmin = (props) => {
  ...
  function reset() {}
  function closeAll() {
    modalIsOpenAddIntakeHistory(false);
  }
  const handleUpdate = () => {
    setuserid(JSON.parse(localStorage.getItem("user")));
  ...
  const handleCancel = () => {
    setuserid(JSON.parse(localStorage.getItem("user")));
    // put(`visit/sendCancelAppointmentEmail/${vid}`, {
    //   visitId: vid,
    //   UserId: userid,
    // });
    put(`/visit/updateCancelVisitStatus/${vid}`, {
      Id: vid,
      Status: 10,
    });
            ...
            <div className="card shadow px-3">
              <div className="row">
                <div className="col-lg-2 col-md-3 col-sm-3 my-1 text-info font-weight-bold text-center">
                  ...
                  <h5 className="font-weight-bold m-0   px-1">
                  <img
                     onClick={() => { 
                      setPatientNationalID(data.patient_NationalID)
                      setPatientName(data.patientFirstName+" "+data.patientLastName)
                      setVisitID(data.id)
                                setIsOpenQrCode(sassTrue)
                            }}
                      src="avatars/qr-code.png"
                      style={{ width: "30px", height: "30px"  }}
                    />
                    <FormControlLabel
                      className="font-weight-bold m-0 pt-1 "
                      control={
                        <Switch
                ...
                <div className="col-lg-4 col-md-6 col-sm-6 my-1 border-left font-weight-bold">
                  <div>
                    ...
                    <p className="text-dark font-weight-bold float-right h5 m-0">
                      {" ( "}
                      {data.meetingtype}
                      {" meeting )"}
                ...
                <div className="col-lg-3 col-md-6 col-sm-6 my-1 border-left pr-md-4 text-center">
                  {showPre ? (
                    <button
                      className={`w-100 btn cc-btn btn-primary mb-2 mt-4  ${
                        data.status == 12 ? "btn-secondary" : "btn-primary"
                      }`}
                  ...
                  <div className="d-flex jusitfy-content-between mb-1">
                    {data.status == 12 ? (
                      <p className="w-100 btn  btn-sm btn-secondary mr-1">
                        Cancel Request Sent
                      </p>
                    ) : (
                      <>
...
export default NewAppointmentCardAdmin;
